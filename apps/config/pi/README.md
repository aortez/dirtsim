...
Configuration files for running DirtSim on a Raspberry Pi with HyperPixel display.

## Files

- `labwc/rc.xml` - Window manager config with touch-to-display mapping
- `labwc/autostart` - Starts dirtsim service after compositor is ready
- `kanshi/config` - Display configuration (rotation, resolution)
- `dirtsim.desktop` - Desktop launcher
