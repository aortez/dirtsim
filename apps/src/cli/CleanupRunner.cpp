...
std::vector<CleanupResult> CleanupRunner::run()
{
    ...
    // Find all dirtsim processes.
    auto serverPids = findProcesses("dirtsim-server");
    auto uiPids = findProcesses("dirtsim-ui");

    SLOG_INFO("Cleaning up dirtsim processes...");
    ...
    for (int pid : serverPids) {
        ...
        result.pid = pid;
        result.processName = "dirtsim-server";
        result.found = true;

        auto start = std::chrono::steady_clock::now();

        SLOG_INFO("→ dirtsim-server (PID {})", pid);
    ...
    for (int pid : uiPids) {
        CleanupResult result;
        result.pid = pid;
        result.processName = "dirtsim-ui";
        result.found = true;

        auto start = std::chrono::steady_clock::now();

        SLOG_INFO("→ dirtsim-ui (PID {})", pid);
