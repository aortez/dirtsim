...
/* Catch configuration errors at compile time - checks if no backend was
 * selected */
#if LV_USE_SDL == 0 && LV_USE_WAYLAND == 0 && LV_USE_LINUX_DRM == 0 && LV_USE_OPENGLES == 0 \
    && LV_USE_X11 == 0 && LV_USE_LINUX_FBDEV == 0
...
void driver_backends_register(void)
{
    ...
    while ((init_backend = available_backends[i]) != NULL) {
        ...
        b = static_cast<backend_t*>(malloc(sizeof(backend_t)));
        LV_ASSERT_NULL(b);

        b->handle = static_cast<backend_handle_t*>(malloc(sizeof(backend_handle_t)));
