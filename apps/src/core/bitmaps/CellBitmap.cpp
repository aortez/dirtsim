...
CellBitmap::CellBitmap(int width, int height)
    : grid_width_(static_cast<int16_t>(width)), grid_height_(static_cast<int16_t>(height))
{
    // Calculate number of 8×8 blocks needed (round up for partial blocks).
    blocks_x_ = static_cast<int16_t>((width + BLOCK_SIZE - 1) / BLOCK_SIZE);
    blocks_y_ = static_cast<int16_t>((height + BLOCK_SIZE - 1) / BLOCK_SIZE);

    // Allocate one uint64_t per 8×8 block, initialized to zero.
    blocks_.resize(static_cast<size_t>(blocks_x_ * blocks_y_), 0);
}

inline void CellBitmap::cellToBlockAndBit(int x, int y, int& block_idx, int& bit_idx) const
{
...
void CellBitmap::set(int x, int y)
{
    int block_idx;
    int bit_idx;
...
void CellBitmap::clear(int x, int y)
{
    int block_idx;
    int bit_idx;
    cellToBlockAndBit(x, y, block_idx, bit_idx);
...
bool CellBitmap::isSet(int x, int y) const
{
    int block_idx;
    int bit_idx;
    cellToBlockAndBit(x, y, block_idx, bit_idx);
...
uint64_t CellBitmap::getBlock(int block_x, int block_y) const
{
    int block_idx = block_y * blocks_x_ + block_x;
    return blocks_[block_idx];
}

bool CellBitmap::isBlockAllSet(int block_x, int block_y) const
{
    return getBlock(block_x, block_y) == 0xFFFFFFFFFFFFFFFFULL;
}

bool CellBitmap::isBlockAllClear(int block_x, int block_y) const
{
    return getBlock(block_x, block_y) == 0;
}

Neighborhood3x3 CellBitmap::getNeighborhood3x3(int x, int y) const
{
    int block_x = x >> 3;
    int block_y = y >> 3;
    int local_x = x & 7;
