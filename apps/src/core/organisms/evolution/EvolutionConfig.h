...
struct EvolutionConfig {
    ...
    int genomeArchiveMaxSize = 1000; // 0 = unlimited; otherwise cap per organismType+brainKind.
    ...
    int robustFitnessEvaluationCount =
        3; // 0 disables extra best-validation; 1+ runs that many validation samples.
...
struct MutationConfig {
    // Mutate a fixed number of weights per offspring, regardless of genome length.
