...
TEST(TrainingPhaseTrackerTest, ImprovementDuringRecoveryKeepsDecayWindowMoving)
{
    TrainingPhaseTracker tracker;
    ...
    ASSERT_EQ(tracker.status().recoveryLevel, 3);

    update = tracker.updateCompletedGeneration(6, 1.35, config);
    EXPECT_TRUE(update.improved);
    EXPECT_FALSE(update.phaseChanged);
    EXPECT_EQ(tracker.status().phase, TrainingPhase::Recovery);
    EXPECT_EQ(tracker.status().lastImprovementGeneration, 6);
    EXPECT_EQ(tracker.status().generationsSinceImprovement, 0);
    EXPECT_EQ(tracker.status().recoveryLevel, 2);

    tracker.updateCompletedGeneration(7, 1.5, config);
    EXPECT_EQ(tracker.status().phase, TrainingPhase::Recovery);
    EXPECT_EQ(tracker.status().recoveryLevel, 1);

    update = tracker.updateCompletedGeneration(8, 1.65, config);
    EXPECT_TRUE(update.improved);
    EXPECT_TRUE(update.phaseChanged);
